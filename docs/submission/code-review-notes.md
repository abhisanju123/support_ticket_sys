...
- API error surfacing (`apiError.js`, notification toasts)
...
1. **Test coverage** — only 2 backend unit test files; no frontend tests.
2. **RBAC** — roles exist on users but endpoints are not role-restricted.
